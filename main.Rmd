...
library(ggplot2)
library(data.table)
dataf <- data.frame(types=factor(c( "logical", "integer", "double", "character"), levels =  c("logical","integer","double","character"), ordered = TRUE), how_general=c(1,2,3,4))
dff <- data.table::melt(as.data.table(dataf), id.var="how_general")
names(dff)[3] <- "types"
ggplot(dff, aes(x=types, y= how_general, group=variable)) + scale_x_discrete(limits=rev) + geom_line(arrow = arrow(type = "closed"), linewidth = 1.75) + geom_point(size = 5, color = "red") + theme(text = element_text(size = 20), axis.text.x = element_text(angle = 45, hjust = 1)) 
...
Reverting these statements is impossible!
...
###R will choose the most general type and convert all the elements to that type.

???

##So even a single character among thousands of doubles will turn all the numbers into characters 

##All the original numbers can be coerced back from characters into numbers

##Attempting that on characters that don't translate to numbers can produce missing values. NA
