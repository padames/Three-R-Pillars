...
When there is a mismatch between the target and the actual preconditions for a use-case even the best of features can look like a well dressed bug! 
...
We colloquially accept that "one will be perhaps more comfortable not knowing some things".
...
.pesenters-notes-small[State of...
...
#### Communicated is an euphemism for "taught" because we are not born using computer languages the way we use human languages  
...
#### The next slides introduce the context to explain it
...
```
???
####Values are stored in data structures to keep them organized in memory
####Data structures make it easier to operate on values

---
class: top, left

#Coercion during data assignment 
...
Our data must go into data structures in memory
